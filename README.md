...
3. В настройках проекта Vercel добавить переменную `DATABASE_URL` со строкой из Neon.
4. Накатить схему на облачную базу локально:
   `DATABASE_URL="<строка из Neon>" npm run db:push`
5. Создать пользователя (MVP работает с одним, id = 1):
   `insert into users (name, email) values ('Имя', 'почта');`
